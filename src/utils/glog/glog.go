...
func (sb *syncBuffer) rotateFile(now time.Time) error {
	if sb.file != nil {
		sb.Flush()
		sb.file.Close()
		if Compress && sb.sev == infoLog {
			// if compress is enabled, and severity is info, start a external process to compress old log file
			// this bzip2 process will not terminate, even current process exit
			_, err := os.StartProcess("/bin/bzip2", []string{"-q", sb.file.Name()}, &os.ProcAttr{})
			if err != nil {
				fmt.Errorf("%v", err)
			}
		}
...
// set the V level, command line will override this setting
func SetV(v Level) error {
	if isFlagSet("v") {
		return nil
	}
	return logging.verbosity.Set(v.String())
}

func SetMaxSize(maxsize uint64) error {
	if !isFlagSet("maxsize") {
		MaxSize = maxsize
	}
	return nil
}

func SetMaxCount(maxcount uint64) error {
	if !isFlagSet("maxcount") {
		MaxCount = maxcount
	}
	return nil
}

func SetLogDir(log_dir *string) error {
	if !isFlagSet("log_dir") {
		logDir = log_dir
		createLogDirs()
		logging.mu.Lock()
		err := logging.createFiles(fatalLog)
		logging.mu.Lock()
		return err
	}
	return nil
